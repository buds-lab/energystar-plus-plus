...
filtered = nycdata %>% 
  filter( LargestPropertyUseType == usetype | 
            X2ndLargestPropertyUseType == usetype | 
            X3rdLargestPropertyUseType == usetype)
filtered1 = filtered %>% 
  mutate(GFA = 
           case_when(
             LargestPropertyUseType %in% usetype ~ LargestPropertyUseTypeGrossFloorArea,
             X2ndLargestPropertyUseType %in% usetype ~ X2ndLargestPropertyUseGrossFloorArea,
             X3rdLargestPropertyUseType %in% usetype ~ X3rdLargestPropertyUseTypeGrossFloorArea,
             TRUE ~ 0)) %>%
  mutate(Age = 2017 - YearBuilt_ll84) %>%
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "multifamily.csv"), row.names = F)
...
filtered = nycdata %>% 
  filter( LargestPropertyUseType %in% usetype | 
            X2ndLargestPropertyUseType %in% usetype | 
            X3rdLargestPropertyUseType %in% usetype)
filtered1 = filtered %>% 
  mutate(GFA = 
           case_when(
             LargestPropertyUseType %in% usetype ~ LargestPropertyUseTypeGrossFloorArea,
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "office.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "retail.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "k12school.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "warehouse.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "hotel.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "worship.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "medical_office.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "hospital.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "residencehall_dormitory.csv"), row.names = F)
...
filtered1 = filtered %>% 
  ...
  filter(GFA > 1000) %>%
  dplyr::select(cols)
write.csv(filtered1, paste0(save_dir, "seniorcare_community.csv"), row.names = F)
...
filtered0 = nycdata %>% 
  filter( LargestPropertyUseType %in% usetype_all | 
            X2ndLargestPropertyUseType %in% usetype_all | 
            X3rdLargestPropertyUseType %in% usetype_all)
...
for (btype in es$btype) {
  ...
  quan = quantile(data1$SourceEUI, probs = c(0.02, 0.98))
  data2 = data1 %>% 
    filter(SourceEUI >= quan[1]) %>%
    filter(SourceEUI <= quan[2])
  
  data3 = data2 %>% 
    #rename(Age = YearBuilt_ll84) %>%
    #mutate(Age = 2017 - Age) %>%
...
#     data1 = data1 %>% filter(SourceEUI <= row$maxEUI)
# 
#   data2 = data1 %>% filter(SourceEUI >= 1 & SiteEUI >= 1)
#   print(paste("min GFA", min(data2$GFA),
#               "min EUI", min(data2$SourceEUI),
#               "max EUI", max(data2$SourceEUI)))
# 
#   filepath = paste0(save_dir1, btype, ".csv")
#   print(paste(filepath, btype, paste(dim(data1), collapse = " x ")))
#   write.csv(data2, filepath, row.names = F)
# }
