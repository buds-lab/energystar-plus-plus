...
exp_C1 <- function(data, int=1) {
  ...
  for(dvar in dvars) {
    ...
    s1 = getModelStats(fit, y, pred)
    df1 = data.frame("exp" = "C1", 
                     "dependent" = dvar, 
                     "model" = "Reg",
                     "interaction" = int,
                     "transform" = "meanCent",
...
exp_C2_old <- function(btype, save_dir, data) {
...
exp_C3b_old <- function(btype, save_dir, data) {
...
exp_C3b <- function(btype, save_dir, data) {
  ...
  for(dvar in dvars) {
    ...
    ## default - 2  
    print(paste(Sys.time(), "xgboost", dvar, "d2"))
    xgfit = tuneXgboost_defaultParams(x0, y, height=2)
    pred = as.numeric(predict(xgfit, as.matrix(x0)))
    s1 = getModelStatsXgboost(xgfit, x0, y, pred)
    df1 = data.frame("exp" = exp_name, 
                     "dependent" = dvar, 
                     ...
                     s1)
    res = rbind(res, df1)
    filepath = paste0(save_dir, paste(exp_name, dvar), as.character(df1$model[1]), ".RDS")
    saveRDS(xgfit, filepath)
    
    
    
    ## default - 3
    print(paste(Sys.time(), "xgboost", dvar, "d3"))
    xgfit = tuneXgboost_defaultParams(x0, y, height=3)
    pred = as.numeric(predict(xgfit, as.matrix(x0)))
    s1 = getModelStatsXgboost(xgfit, x0, y, pred)
    df1 = data.frame("exp" = exp_name, 
                     "dependent" = dvar, 
    ...
    ## random
    print(paste(Sys.time(), "xgboost", dvar, "r"))
    xgfit = tuneXgboost_randomSearch(x0, y)
    pred = as.numeric(predict(xgfit, as.matrix(x0)))
    s1 = getModelStatsXgboost(xgfit, x0, y, pred)
    df1 = data.frame("exp" = exp_name, 
                     "dependent" = dvar, 
    ...
    ## grid - 2
    print(paste(Sys.time(), "xgboost", dvar, "g2"))
    xgfit = tuneXgboost_gridSearch(x0, y, height=2)
    pred = as.numeric(predict(xgfit, as.matrix(x0)))
    s1 = getModelStatsXgboost(xgfit, x0, y, pred)
    df1 = data.frame("exp" = exp_name, 
                     "dependent" = dvar, 
    ...
    ## grid - 3
    print(paste(Sys.time(), "xgboost", dvar, "g3"))
    xgfit = tuneXgboost_gridSearch(x0, y, height=3)
    pred = as.numeric(predict(xgfit, as.matrix(x0)))
    s1 = getModelStatsXgboost(xgfit, x0, y, pred)
    df1 = data.frame("exp" = exp_name, 
                     "dependent" = dvar, 
    ...
    ## adaptive
    print(paste(Sys.time(), "xgboost", dvar, "a"))
    xgfit = tuneXgboost_adaptiveSearch(x0, y)
    pred = as.numeric(predict(xgfit, as.matrix(x0)))
    s1 = getModelStatsXgboost(xgfit, x0, y, pred)
    df1 = data.frame("exp" = exp_name, 
                     "dependent" = dvar, 
