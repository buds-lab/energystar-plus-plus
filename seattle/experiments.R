...
exp_C1 <- function(data) {
  ...
  df1 = data.frame("name" = "C1", "interaction" = 1, "model" = "Reg",
                   "dependent" = dvar, s1)
  ...
  df2 = data.frame("name" = "C1", "interaction" = 1, "model" = "Reg",
                   "dependent" = dvar, s2)
...
exp_C2 <- function(btype, save_dir, data) {
  ...
  for(int in 2:int_depth) {
    ...
    s1 = getModelStats(fit1, data$SourceEUI)
    df1 = data.frame("name" = "C2", "interaction" = int, "model" = "RegInt",
                     "dependent" = dvar, s1)
    ...
    saveRDS(fit1, filepath)
    s2 = getModelStats(fit2, data$SiteEnergyUse)
    df2 = data.frame("name" = "C2", "interaction" = int, "model" = "RegInt",
                     "dependent" = dvar, s2)
...
exp_C2_reg_task <- function(dvar, int) {
  ...
  s1 = getModelStats(fit1)
  df1 = data.frame("name" = "C2", "interaction" = int, "model" = "Reg",
                   "dependent" = dvar, s1)
...
exp_C2_parallel <- function(btype, save_dir, data) {
  ...
  int_depth = length(ivars)
  params <- expand.grid(dvar=dvars, int=2:int_depth, stringsAsFactors = F)
...
btypes = c("school", "office", "hotel", "multifamily",
...
for(btype in btypes) {
  ...
  #write.csv(cc, paste0(save_dir, btype, "_result.csv"))
}
