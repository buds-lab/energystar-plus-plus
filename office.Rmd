...
for (intr in 1:intr_depth) {
  ...
  print(paste(Sys.time(), "xgboost adaptive search", y, intr))
...
for (intr in 1:intr_depth) {
  ...
  print(paste(Sys.time(), "xgboost random search", y, intr))
...
allMetrics0 = allMetrics %>%
  ...
  mutate(algo = stringr::str_sub(model, 1, 4)) %>%
  filter(dependent == "SOURCE_EUI")
